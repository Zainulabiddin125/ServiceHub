...
//// ── Email Service ─────────────────────────────────────────────────────────
//// TEMPORARILY DISABLED: Real email sender is commented out while diagnosing
//// the login hang issue on the production server. The no-op sender below
//// keeps Identity happy without making any SMTP calls. Once login is confirmed
//// working, re-enable the two lines below and remove the NoOpEmailSender line.
////
//// builder.Services.Configure<SmtpSettings>(builder.Configuration.GetSection("SmtpSettings"));
//// builder.Services.AddTransient<Microsoft.AspNetCore.Identity.UI.Services.IEmailSender, EmailSender>();
